...
#define MAX_LIST_SIZE   10000000000
...
void *thread_work(void *id){
    ...
    double curr_time = (curr.tv_sec)+0.000000001*(curr.tv_nsec);
    if (DEBUG) printf("barrier1 after initial sort of %d thread: %8.4f\n",my_id,curr_time);
    for (int level = 0; level < q; level++) {
        ...
        clock_gettime(CLOCK_REALTIME, &curr);
        curr_time = (curr.tv_sec)+0.000000001*(curr.tv_nsec);
        if (DEBUG) printf("before level %d of %d thread: %8.4f\n",level,my_id,curr_time);
        pthread_barrier_wait(&barrier2);
        clock_gettime(CLOCK_REALTIME, &curr);
        curr_time = (curr.tv_sec)+0.000000001*(curr.tv_nsec);
        if (DEBUG) printf("after level %d of %d thread: %8.4f\n",level,my_id,curr_time);
        // Copy work into list for next itertion
...
if (DEBUG) print_list(list, list_size);
    for (my_id = 0; my_id < num_threads; my_id++) {
    // Sort list
        status = pthread_create(&p_threads[my_id],&attr,thread_work, (void *)(size_t) my_id);
        if (status != 0){
...
// VS: ... parallel merge sort
    ...
    int rc = pthread_barrier_init(&barrier1, NULL, num_threads);

    if (rc) {
        fprintf(stderr, "pthread_barrier_init: %s\n", rc);
        exit(1);
    }
    rc = pthread_barrier_init(&barrier2, NULL, num_threads);

    if (rc) {
        fprintf(stderr, "pthread_barrier_init: %s\n", rc);
        exit(1);
    }
    rc = pthread_barrier_init(&barrier3, NULL, num_threads);
    if (rc) {
        fprintf(stderr, "pthread_barrier_init: %s\n", rc);
        exit(1);
    }
    parallel_sort_list(q);
