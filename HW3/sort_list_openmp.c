...
if (DEBUG) print_list(list, list_size); 
    ...
    #pragma omp parallel for default(shared) private(my_id)
    for (my_id = 0; my_id < num_threads; my_id++) {
    ...
    for (level = 0; level < q; level++) {
        #pragma omp parallel for default(shared) firstprivate(level) private(my_id,my_blk_size,my_own_blk,my_own_idx,my_search_blk,my_search_idx,my_search_idx_max,my_write_blk,my_write_idx,idx,my_search_count,i)
