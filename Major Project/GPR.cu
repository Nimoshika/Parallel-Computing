...
__global__ void compute_K_matrix_gpu (int num_threads, double *K, struct node_coord *grid_points, int m)
{
    ...
    for(int i = threadIdx.x; i < n; i+= num_threads)
    {
        for (int j = 0; j < n; j++)
        {
            ...
            K[i*n + j] = (1/sqrt(2*3.1415)) * exp(-0.5 * (temp_x + temp_y));
            if (i == j)
                K[i*n + j] += NOISE;
        }
    }
...
void compute_k (double **k, struct node_coord *rstar, struct node_coord *grid_points, int n)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        k[0][i] = (1/sqrt(2*3.1415)) * exp(-0.5 * (temp_x + temp_y));
    }
}
...
int main(int argc, char* argv[])
{
    ...
    if(argc != 5)
    {
        printf("Invalid number of input arguements. Please execute the binary as ./a.out m x* y* BLOCK_SIZE*\n");
        return 0;
    }
    else
    {
    ...
    n = m * m;
    h = 1.0 / (double)(m + 1);
    int num_threads = BLOCK_SIZE*BLOCK_SIZE;//getNumThreads();
    ...
    int blockdim = BLOCK_SIZE*BLOCK_SIZE;
    device_run_chol<<<1,blockdim>>>(blockdim, K1, L, partial_sum, n);
